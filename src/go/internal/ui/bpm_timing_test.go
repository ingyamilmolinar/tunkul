...
func TestBPMFunctional60(t *testing.T) {
    ...
    var times []time.Time
    g.highlightHook = func(row, idx int) { if row == 0 { times = append(times, time.Now()) } }

    // Set BPM=60 (1 beat/sec), apply it, then start playback.
    g.drum.SetBPM(60)
    applyUntil := time.Now().Add(200 * time.Millisecond)
    for time.Now().Before(applyUntil) { _ = g.Update(); time.Sleep(17 * time.Millisecond) }

    g.drum.playPressed = true
    deadline := time.Now().Add(8 * time.Second)
    for time.Now().Before(deadline) { _ = g.Update(); time.Sleep(17 * time.Millisecond) }

    if len(times) < 3 { t.Fatalf("insufficient events captured: %d", len(times)) }
    var intervals []time.Duration
    for i := 2; i < len(times); i++ { intervals = append(intervals, times[i].Sub(times[i-1])) }
    if len(intervals) == 0 { t.Fatalf("no intervals computed; times=%v", times) }
    for i, d := range intervals {
        if d < 920*time.Millisecond || d > 1080*time.Millisecond {
...
func TestBPMStressShortSegmentsHighBPM(t *testing.T) {
    ...
    for time.Now().Before(applyUntil) { _ = g.Update(); time.Sleep(5 * time.Millisecond) }

    // Capture per-row audio callback times.
    type rec struct{ id string; t time.Time }
    var times []rec
    ...
    for _, r := range times { got[r.id] = append(got[r.id], r.t) }
    if len(got) == 0 {
...
func TestHighlightAudioSync25ms(t *testing.T) {
    ...
    g.highlightHook = func(row, idx int) { if row == 0 { hlTimes = append(hlTimes, time.Now()) } }
    ...
    n := len(hlTimes)
    if len(auTimes) < n { n = len(auTimes) }
    hl := hlTimes[1:n]
    au := auTimes[1:n]
