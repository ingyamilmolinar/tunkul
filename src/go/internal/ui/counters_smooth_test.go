...
func TestCountersAdvanceSmoothly(t *testing.T) {
	...
	for i := 0; i < samples; i++ {
		...
		time.Sleep(15 * time.Millisecond) // approximate frame pacing
	}
	...
	for i := 1; i < len(beats); i++ {
		...
		// At 120 BPM, 1 beat = 0.5s. With ~15ms frames, delta ~= 0.03 beats.
		// Accept up to 0.2 beats to allow scheduling jitter in CI.
	...
	for i := 1; i < len(mss); i++ {
		...
		// With ~15ms frame pacing, expect deltas around that magnitude.
		if d := mss[i] - mss[i-1]; d < 1 || d > 80 {
