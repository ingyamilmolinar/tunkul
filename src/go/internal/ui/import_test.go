...
func TestImportScalesSubdivisionsAndKeepsOrthogonality(t *testing.T) {
    ...
    a := g.nodeAt(0,16); b := g.nodeAt(16,16); c := g.nodeAt(16,32)
    if a == nil || b == nil || c == nil { t.Fatalf("missing nodes after import") }
    if a.J != b.J { t.Fatalf("not horizontal: a.J=%d b.J=%d", a.J, b.J) }
    if b.I != c.I { t.Fatalf("not vertical: b.I=%d c.I=%d", b.I, c.I) }
    if g.drum.BPM() != 110 { t.Fatalf("bpm not imported: %d", g.drum.BPM()) }
}
