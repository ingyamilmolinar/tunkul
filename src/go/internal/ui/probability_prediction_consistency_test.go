...
func TestProbabilityPreviewMatchesPrediction_RewindAndForward(t *testing.T) {
	...
	g.drum.Offset = 0
	g.refreshDrumRow()
	t.Logf("pred0=%v", g.predVisibleByRow[0][0:window])
	...
	for _, off := range []int{0, 7, 19, 37} {
		...
		want := append([]bool(nil), g.predVisibleByRow[0][off:off+window]...)
...
func TestProbabilityAudibleRateRoughlyMatchesP(t *testing.T) {
	...
	for i := 0; i < horizon; i++ {
		...
		if i < 32 {
			t.Logf("i=%d node=%d type=%d audible=%v", i, bi.NodeID, bi.NodeType, g.predAudibleByRow[0][i])
		}
		if bi.NodeID == A.ID {
			occ++
			if g.predAudibleByRow[0][i] {
				hits++
			}
		}
	}
...
func TestProbabilityWithEverySkip_WindowsMatchPrediction(t *testing.T) {
	...
	for off := 0; off+win <= horizon; off += 7 {
		...
		g.refreshDrumRow()
		got := append([]bool(nil), g.drum.Rows[0].Steps...)
		want := append([]bool(nil), g.predVisibleByRow[0][off:off+win]...)
