...
// move in a few steps
for (let i = 1; i <= 5; i++) {
  const yy = downY + Math.floor(50 * (i/5));
  await page.evaluate(({x,y}) => {
    const c = document.querySelector('canvas');
...
const minY = 120;
const maxY = winH - 120;
if (endY < minY || endY > maxY) {
  throw new Error(`splitter out of bounds: ${endY} not in [${minY}, ${maxY}]`);
}
...
if (drumY !== endY) {
  throw new Error(`drum view not aligned with splitter: drumY=${drumY} splitY=${endY}`);
}
