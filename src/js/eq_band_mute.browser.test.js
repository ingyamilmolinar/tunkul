...
// Test: muting ONE band should NOT silence entire audio; muting ALL bands should.
...
const result = await page.evaluate(async () => {
  ...
  // all-muted → gain=0 short-circuit; otherwise serial peaking filters with
  // muted bands getting -60dB cascaded twice.
  async function renderWithEQ(bands) {
    ...
    // All-muted short-circuit: gain=0 (matches audio.js:910-914)
    const allMuted = bands.every((b) => b?.muted === true);
    if (allMuted) {
      const silence = offline.createGain();
      silence.gain.value = 0;
      src.connect(silence);
      silence.connect(offline.destination);
      src.start(0);
      const rendered = await offline.startRendering();
      return { rms: 0, samples: rendered.length };
    }
  ...
  // All-unmuted band config (10 bands)
  ...
  const baseline = await renderWithEQ(allUnmuted);

  // 2) ONE band muted (band 4: 354-707 Hz mid range — kick has energy here but also elsewhere)
  const oneBandMutedConfig = allUnmuted.map((b, i) => ({
    ...b, muted: i === 4,
  }));
  const oneBandMuted = await renderWithEQ(oneBandMutedConfig);

  // 3) ALL bands muted — should be near-silent
  const allMutedConfig = Array.from({ length: 10 }, () => ({
    gainDB: 0, muted: true,
  }));
  const allMuted = await renderWithEQ(allMutedConfig);

  return {
    baselineRMS: baseline.rms,
    baselineSamples: baseline.samples,
    oneBandMutedRMS: oneBandMuted.rms,
    oneBandMutedRatio: baseline.rms > 0 ? oneBandMuted.rms / baseline.rms : 0,
    allMutedRMS: allMuted.rms,
  };
});

if (isCoverageEnabled()) await flushCoverage(page, new URL("../../coverage/browser-raw", import.meta.url).pathname, "eq_band_mute");
await browser.close();
server.close();

console.log("Test results:");
console.log(`  Baseline RMS:        ${result.baselineRMS.toFixed(6)} (${result.baselineSamples} samples)`);
console.log(`  One band muted RMS:  ${result.oneBandMutedRMS.toFixed(6)} (ratio=${result.oneBandMutedRatio.toFixed(4)})`);
console.log(`  All bands muted RMS: ${result.allMutedRMS.toFixed(6)}`);

// Verify baseline has audio
if (result.baselineRMS < 0.001) {
  throw new Error(`Baseline RMS too low (${result.baselineRMS}), synth sample may not be rendered`);
}

// KEY TEST: Muting ONE band should NOT silence the entire output.
// The kick drum has energy across multiple frequency bands, so muting only
// the mid band should still produce substantial audio output.
if (result.oneBandMutedRatio < 0.01) {
  throw new Error(
    `FAIL: Muting ONE band silenced the signal (ratio=${result.oneBandMutedRatio.toFixed(4)}). ` +
    `Expected partial reduction, not near-silence. ` +
    `This indicates the bug where single band mute kills all audio.`
  );
}

// Verify ALL bands muted produces near-silence
if (result.allMutedRMS > 0.0001) {
  throw new Error(
    `All-muted RMS should be near zero, got ${result.allMutedRMS}`
  );
}

console.log("PASS: EQ band mute test passed");
