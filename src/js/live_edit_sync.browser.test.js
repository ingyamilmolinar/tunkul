...
async function snapshot(row) {
  const [idxs, off, len, steps] = await Promise.all([
...
function validateInWindow(state, row) {
  const last = Math.max(0, state.idxs[row] - 1);
  if (last < state.off || last >= state.off + state.len) throw new Error(`row ${row} last not in window`);
}

function* futureAbs(state, row, beats, subdiv) {
  const start = Math.max(0, state.idxs[row]);
...
await page.evaluate(() => {
  addNode(3,0,'regular'); addNode(3,1,'regular');
...
const beatsAhead = 1;
await page.evaluate(async ({beats, subdiv}) => {
...
// Probability edit sequence: toggle to probability and verify future sync
{
  const browser2 = await chromium.launch({ args: ["--autoplay-policy=no-user-gesture-required"] });
  const page2 = await browser2.newPage();
  await page2.goto(`http://localhost:${port}/`).catch(()=>{}); // server closed; re-create
}
