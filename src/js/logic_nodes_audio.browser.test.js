...
async function scenario({ type, logicKind, logicP }) {
  await goto();
  await waitReady();
  // Build two-node path at offset grid coords to avoid default
  await page.evaluate(({type}) => {
    // types: 'regular' | 'silent' | 'mute'
    const t = type;
  ...
  if (logicKind) {
    await page.evaluate(({kind, p}) => setNodeLogicGrid(20, 0, kind, 0, p ?? 0), { kind: logicKind, p: logicP });
