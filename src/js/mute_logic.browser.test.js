...
const setup = await page.evaluate(() => {
  resetAudioDebug();
  setFollow(false);
  const startId = addNode(0, 0, "regular");
  const muteId = addNode(1, 0, "mute");
  const tailId = addNode(2, 0, "regular");
  addEdgeGrid(0, 0, 1, 0);
  addEdgeGrid(1, 0, 2, 0);
  addEdgeGrid(2, 0, 0, 0);
  setNodeLogicGrid(1, 0, "every_n_triggers", 2, 0);
  updateBeatInfosJS();
  const cycleLen = rowBeatCount(0);
  let muteIdx = -1;
  let tailIdx = -1;
  for (let i = 0; i < cycleLen; i++) {
    const info = beatInfoAt(0, i);
    if (!info) continue;
...
if (setup.muteIdx < 0 || setup.tailIdx < 0) {
  throw new Error(`failed to locate mute/tail indices: ${JSON.stringify(setup)}`);
}

await page.waitForTimeout(250);

const pattern = await page.evaluate(({ cycleLen, tailIdx }) => {
  const values = [];
  const ensureVisible = (idx) => { ensure(idx + 1); return !!visibleAt(0, idx); };
  for (let cycle = 0; cycle < 4; cycle++) {
    const idx = tailIdx + cycle * cycleLen;
...
if (pattern.length !== expected.length || pattern.some((v, i) => v !== expected[i])) {
  throw new Error(`mute logic mismatch: got=${pattern.join(",")} want=${expected.join(",")}`);
}

const triggeredPattern = await page.evaluate(({ cycleLen, muteIdx }) => {
  const values = [];
  for (let cycle = 0; cycle < 4; cycle++) {
    const idx = muteIdx + cycle * cycleLen;
    ensure(idx + 1);
...
if (triggeredPattern.length !== expectedTriggers.length || triggeredPattern.some((v, i) => v !== expectedTriggers[i])) {
  throw new Error(`mute trigger mismatch: got=${triggeredPattern.join(",")} want=${expectedTriggers.join(",")}`);
}
