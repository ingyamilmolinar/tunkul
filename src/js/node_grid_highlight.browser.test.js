...
try {
  ...
  // Build a 1x1 loop, enable simple draw, and start.
  await page.evaluate(() => { if (typeof setSimpleDraw === 'function') setSimpleDraw(true); buildPerfRect(1, 1); setBPM(150); startPlay(); });
  ...
  for (let t = 0; t < 30 && !ok; t++) {
    ...
    ok = await page.evaluate((coords) => {
      ...
      for (const [i,j] of coords) {
        if (nodeHighlightedAt(i,j)) return true;
      }
      return false;
    }, coords);
  }
