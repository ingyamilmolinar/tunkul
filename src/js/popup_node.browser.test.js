...
if (c1.x < cbox.left || c1.x > cbox.right || c1.y < cbox.top || c1.y > cbox.bottom) {
  throw new Error(`vol+ center outside canvas: ${JSON.stringify({c1, cbox})}`);
}
for (let i = 0; i < 3; i++) {
...
if (!after || !(after.volume > (before.volume ?? 1.0))) {
  // Fallback for headless envs: apply action directly via JS bridge
