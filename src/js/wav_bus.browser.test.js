...
<script>
  ...
  window.__ready = new Promise(r => {
    const iv = setInterval(() => { if (typeof startPlay === 'function') { clearInterval(iv); r(true); } }, 10);
  });
</script>`;
...
try {
  ...
  await page.evaluate(() => {
    window.registerWav('snare', 'data:audio/wav;base64,'); // ignored; render path used
  });
  ...
  if (buses < 8 || buses > 32) {
    throw new Error('unexpected number of volume buses: ' + buses);
  }
} finally {
