...
// Phase E (Phase F prep): Allow per-knob runtime profile overrides via env.
// Pass a JSON object in BEATMO_PROFILE_OVERRIDE; it's injected as
// window.__beatmoProfileOverride before WASM init so newRuntimeProfile()
// reads it.
//
// Example:
//   BEATMO_PROFILE_OVERRIDE='{"drawMinIntervalMS":0}' make bench-browser
let profileOverride = null;
if (process.env.BEATMO_PROFILE_OVERRIDE) {
  try {
    profileOverride = JSON.parse(process.env.BEATMO_PROFILE_OVERRIDE);
    console.log(`[BENCH] runtime profile override: ${JSON.stringify(profileOverride)}`);
  } catch (e) {
    console.warn(`[BENCH] invalid BEATMO_PROFILE_OVERRIDE JSON: ${e.message}`);
  }
}
...
for (const bpm of BPM_LEVELS) {
  ...
  results.push({
    ...
    liveImages: stats?.liveImages ?? 0,
    imagesAllocatedTotal: stats?.imagesAllocatedTotal ?? 0,
    imagesByTag: stats?.imagesByTag ?? "",
  });
  console.log(`  imagesByTag: ${stats?.imagesByTag ?? "(none)"}`);
...
const jsonResults = results.map(r => ({
  ...
  liveImages: r.liveImages ?? 0,
  imagesAllocatedTotal: r.imagesAllocatedTotal ?? 0,
  imagesByTag: r.imagesByTag ?? "",
}));
