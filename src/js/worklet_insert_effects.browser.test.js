...
try {
  ...
  await page.waitForFunction(() =>
    typeof addInsertEffect === "function" &&
    typeof removeInsertEffect === "function" &&
    typeof getInsertEffects === "function" &&
    typeof insertEffectCatalog === "function" &&
    typeof setInsertEffectParam === "function" &&
  ...
  await page.evaluate(() => {
    const slots = getInsertEffects("kick") || [];
    for (let i = slots.length - 1; i >= 0; i--) removeInsertEffect("kick", i);
  });
  ...
  async function captureAudio(page, effectType, effectParams, retryAttempt) {
    return await page.evaluate(async ({ effectType, effectParams, retryAttempt, NOISE_FLOOR }) => {
      ...
      // Clean slate
      const existing = getInsertEffects("kick") || [];
      for (let i = existing.length - 1; i >= 0; i--) removeInsertEffect("kick", i);

      // Add effect if specified
      if (effectType) {
        const idx = addInsertEffect("kick", effectType);
        if (effectParams) {
          for (const [name, value] of Object.entries(effectParams)) {
            setInsertEffectParam("kick", idx, name, value);
          }
        }
        // Verify effect was added
        const check = getInsertEffects("kick") || [];
        if (check.length === 0) {
